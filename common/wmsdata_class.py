   ...
   def aggregate_on_user(self):
      ...
      for i in range(1,len(self.userlist)):
         for key in self.userlist[i].userdata:
            if key != 'dn' and key != 'voms_group' and key != 'role' and key != 'VO' and key != 'WMP_in_col_min_nodes' and key != 'WMP_in_col_max_nodes':
            ...
            elif key == 'WMP_in_col_min_nodes':
                  print key, ':', self.wmstotal_dict[key], ':' , self.userlist[i].userdata[key]
                  if self.wmstotal_dict[key] != None and self.wmstotal_dict[key] != 'Null':
                     self.wmstotal_dict[key] = str(min(int(self.wmstotal_dict[key]),int(self.userlist[i].userdata[key])))
                  else:
                     self.wmstotal_dict[key] = str( int(self.userlist[i].userdata[key]) )
            elif key == 'WMP_in_col_max_nodes':
                  print key, ':', self.wmstotal_dict[key], ':' , self.userlist[i].userdata[key]
                  if self.wmstotal_dict[key] != None and self.wmstotal_dict[key] != 'Null':
                     self.wmstotal_dict[key] = str(max(int(self.wmstotal_dict[key]),int(self.userlist[i].userdata[key])))
                  else:
                     self.wmstotal_dict[key] = str( int(self.userlist[i].userdata[key]) )
