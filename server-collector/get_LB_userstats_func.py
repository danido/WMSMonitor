...
def get_LB_userstats(wmshost,lbhost,send_file_OID,port):

   logger = logging.getLogger('get_LB_userstats')


# SNMPv2-SMI::enterprises.10403.94.101.1 = STRING: "FILE CREATED = /root/wmsmon/bin/CE_MM.txt"

   filename = 'out_userstat_' + wmshost + '_'  + lbhost + '_' + str(time.time()) + '.txt'
   ...
   logger.info('We can now try to get the userstats file')
   ...
   cmd = "echo " + wmshost + " " + lbhost + " > /var/www/html/wmsmon/tmp/userstat_request.txt"
   os.system(cmd)
