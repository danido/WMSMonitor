...
int main(int argc,char **argv) {
	...
	cond[1].value.i = EDG_WLL_EVENT_MATCH;

        cond[2].attr =  EDG_WLL_QUERY_ATTR_SOURCE;
        cond[2].value.i = EDG_WLL_SOURCE_WORKLOAD_MANAGER;
...
//   printf("MATCHED CE\n");
//   printf("OWNER\tJOBID\tDESTINATION_CE\tTIMESTAMP\n");
	for (i=0; events[i].type; i++) {
		edg_wll_Event	*e = events+i;

		if ((strcmp(e->match.dest_id,"localhost")!=0) && (strcmp(e->match.dest_id,"unavailable")!=0)) {
			char	*job = glite_jobid_unparse(e->any.jobId);
                        printf("%s\t%s\t%s\t%s",
                                e->any.user,
                                job,
                                e->match.dest_id,                          
                                ctime(&e->any.timestamp.tv_sec));
			free(job);
		}
	}
