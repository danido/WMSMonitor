...
//printf("to: %s\n",asctime(&tm));
	...
	cond[1].value.i = EDG_WLL_EVENT_REGJOB;
	...
	if (edg_wll_QueryEventsProxy(ctx,NULL,cond,&events)) {
        ...
        /* SELECTION OF "SUBMITTED SINGLE JOBS" */
//        printf("SELECTION OF SUBMITTED SINGLE JOBS\n");
//        printf("OWNER\tJOBID\tNSUBJOBS\tTIMESTAMP\n"); 
	for (i=0; events[i].type; i++) {
		edg_wll_Event	*e = events+i;

		if (e->regJob.nsubjobs == 0) {
			char	*job = glite_jobid_unparse(e->any.jobId);
                        printf("%s\t%s\t0\t%s",
                                e->any.user,
                                job,
                                ctime(&e->regJob.timestamp.tv_sec)
                                );
                        //print();

			free(job);
		} else if(e->regJob.nsubjobs > 0) {
			char	*job = glite_jobid_unparse(e->any.jobId);
                        printf("%s\t%s\t%d\t%s",
                                e->any.user,
                                job,
